# the same like mnist but with dataset of celeb faces
...
def train():
    ...
    transform = transforms.Compose(
        [
            ...
            transforms.CenterCrop(IMAGE_SIZE), # center crop image to 64x64
    ...
    dataset = datasets.CelebA(root="celeb_dataset2/", split="train", download=True, transform=transform)
    ...
    torch.save(gen.state_dict(), "gen2.pth")
    torch.save(disc.state_dict(), "disc2.pth")
    writer_real.close()
    writer_fake.close()
...
def genrate_image():
    ...
    gen.load_state_dict(torch.load("gen2.pth", map_location=torch.device('cpu')))
    gen.eval()
